  ...
  return (
    <>
      ...
      {(selectedDay || isPopupClosing) && selectedDay && createPortal(
        <div
          className={`cal-day-popup ${inlineMode ? 'popup-left' : 'popup-right'} ${isPopupClosing ? 'cal-popup-exit' : 'cal-popup-enter'}`}
      ...
      <style>{`
        ...
        .cal-popup-enter {
          animation: calPopIn 0.25s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
        .cal-popup-exit {
          animation: calPopOut 0.2s cubic-bezier(0.4, 0, 0.2, 1) forwards;
        }
        @keyframes calPopIn {
          0% { opacity: 0; transform: translateY(-50%) scale(0.9); margin-top: 10px; }
          100% { opacity: 1; transform: translateY(-50%) scale(1); margin-top: 0px; }
        }
        @keyframes calPopOut {
          0% { opacity: 1; transform: translateY(-50%) scale(1); margin-top: 0px; }
          100% { opacity: 0; transform: translateY(-50%) scale(0.9); margin-top: 10px; }
        }
